...
@section('content')
    <div class="bg-gray-50">
        <div class="mx-auto max-w-2xl px-4 pb-10 pt-5 sm:px-6 lg:max-w-7xl lg:px-8">
            <!-- Detail Produk -->
            <div class="lg:mt-0">
                <h2 class="text-lg font-medium text-gray-900">Pembelian #{{ $datas->code_invoice }}</h2>
                <div class="flex justify-between items-center text-xs my-2 font-medium text-gray-500">Dibeli pada tanggal
                    {{ $datas->created_at->translatedFormat('d F Y') }} <span
                    ...
                    <div
                        ...
                        <div class="">
                            <img class="flex size-10 flex-none object-cover items-center justify-center rounded-lg"
                                src="{{ $datas->seller->imageLink() }}">
                        </div>
                        <div class="ml-4 flex-auto">
                            <p class="text-sm font-medium text-gray-700">{{ $datas->seller->shop_name }}</p>
                            <p class="text-xs text-gray-500 line-clamp-1">{{ $datas->seller->location }}</p>
                        </div>
                    </div>
                </div>
